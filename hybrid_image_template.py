...
def choose_transform_shape(image_shape, kernel_shape, engine):
    ...
    R,C = image_shape[0],image_shape[1]
    Kr,Kc = kernel_shape[0],kernel_shape[1]
    full_height = R + Kr + -1
    full_width = C + Kc - 1
    ...
    if engine.name == "fft":
        ...
        # return
        full_height = next_power_of_two(full_height)
        full_width = next_power_of_two(full_width)
...
def hybrid_image(low_image, high_image, kernel, engine):
    ...
    if low_image.ndim == 3 and low_image.shape[2] == 3:
        ...
        #     result = hybrid_plane(low_image[:,:,channel],
        #                           high_image[:,:,channel],kernel,engine)
        #     planes.append(result)
        # return np.stack(planes,axis=2)
        #Alternate soln
        result = np.zeros((low_image.shape[0],low_image.shape[1],3),dtype = np.float64)
        for channel in range(3):
            result[:,:,channel] = hybrid_plane(low_image[:,:,channel],
                                               high_image[:,:,channel], kernel, engine)
        return result
